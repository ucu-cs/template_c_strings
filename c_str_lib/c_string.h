 ...
 * Note: if you implemented the tip above, returns (capacity_m-1) because one byte is reserved for the \0 symbol.
 * If str == NULL should return 0.
 */
 ...
 *      RANGE_ERR if the index is bad -- out of the range [0, str->size_m).
 */
 ...
 *
 * If the buffer data_m has one reserved byte (maximal size_m == capacity_m - 1), implementation
 * can just put the \0 char at the end of the actual string and return the data_m pointer.
 * 
 * Returns:
 ...
 *
 * Note 1: if !reserve (reserve == false) -- should reallocate only if to->data_m memory block
 ...
 *
 * Note: Insert at the end -- position data_m[size_m] is allowed.
 *
 * Returns:
 *      0 if OK,
 *      NULL_PTR_ERR if str is NULL,
 *      RANGE_ERR if position is bad -- out of the range [0, size_m],
